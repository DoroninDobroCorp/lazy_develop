...
3.  **Формат Ответа:**
    ...
    *   **Верификация:** Когда считаешь, что выполнил последний шаг плана и код надо проверить, добавь ТРЕТИЙ блок: ```verify_run``` (без текста внутри). Сам блок — сигнал системе запустить проект и собрать логи.
...
2.  **ПРАВИЛО ПУТЕЙ (САМОЕ ВАЖНОЕ!):**
    ...
    *   **Пример:** Если проект находится в папке `/path/to/my-project`, и тебе нужен файл `src/app.js`:
        ...
        *   Путь не должен начинаться с имени корневой папки проекта.
        *   Разрешённые символы в пути: латиница/цифры/`_`/`-`/`.` и разделитель `/`.
    *   **Обязательный формат пути:** `path="relative/path/to/file"`. Путь должен быть указан в кавычках и относительно корня проекта.
...
6.  **ПРАВИЛО ЛОГИРОВАНИЯ (КРИТИЧЕСКИ ВАЖНО):**
      ...
      *   Это правило абсолютно, исключений нет.
"""
    return f"""Ты — AI-ассистент в автоматизированной системе. Твоя задача — анализировать код и генерировать команды для его изменения.
{execution_rules}
{global_rules}
{boundary_instr}
"""
...
Проанализируй текущий код с учётом истории и исходной цели. Главный вопрос: решена ли основная задача пользователя?
Если да, то мелкие стилистические улучшения или рефакторинг не требуются. Сразу пиши `ГОТОВО` и `done_summary`.
Продолжай работу только если основная функциональность ещё не реализована или есть критические ошибки, которые помешают коду работать.
